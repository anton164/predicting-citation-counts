...
class JournalExperiment(Experiment):
    def __init__(self, pandas_df):
        ...
        pandas_df[["FirstPage", "LastPage"]] = pandas_df[["FirstPage", "LastPage"]].apply(pd.to_numeric, errors='coerce')
        pandas_df = pandas_df.dropna(subset=["FirstPage", "LastPage"])
        pandas_df = pandas_df.assign(PageCount=pandas_df.apply(lambda doc: doc["LastPage"] - doc["FirstPage"], axis=1))
        ...
        categorials = pandas_df[["AuthorProminence", "CitationCount", "FieldOfStudy_1", "MagBin"]]
        encoded_categories = pd.get_dummies(categorials, dummy_na=True)

        # Add encoded cats back
        self.X = self.X.drop(columns=[["AuthorProminence", "CitationCount", "FieldOfStudy_1", "Journal", "MagBin", "Rank"]])
    ...
    def train(self):
        ...
        self.models = [
            ("Logistic Regression", LogisticRegression(1).fit(self.X_train, self.y_train)),
            ("Linear SVM", LinearSVC(C=0.1, max_iter=50).fit(self.X_train, self.y_train)),
    ...
    def evaluate(self):
        ...
        st.dataframe(
            pd.DataFrame(data=np.array(model_predictions).reshape(count_samples, len(model_predictions)), columns=model_prediction_labels)
        )
